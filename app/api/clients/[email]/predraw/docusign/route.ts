...
export async function POST(
  ...
  try {
    ...
    console.log("🔵 Marking predraw DocuSign complete for clientId:", clientData.id, "outfitterId:", outfitterId);
    ...
    const { data: updateData, error: updateError } = await adminClient
      .from("client_predraw_submissions")
      .update({ docusign_status: "completed" })
      .eq("id", existing.id)
      .select();

    if (updateError) {
      console.error("Error updating DocuSign status:", updateError);
      return NextResponse.json({ error: updateError.message }, { status: 500 });
    }

    console.log("✅ Updated predraw DocuSign status:", updateData);

    return NextResponse.json({ success: true, updated: updateData });
  } catch (e: any) {
    console.error("Error in docusign update:", e);

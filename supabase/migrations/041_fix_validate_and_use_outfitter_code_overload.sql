-- Fix: "Could not choose the best candidate function between" validate_and_use_outfitter_code
-- Two overloads existed (2-param and 3-param with p_email), causing ambiguity when calling.
-- Drop both and create a single function with optional p_email (DEFAULT NULL).

DROP FUNCTION IF EXISTS validate_and_use_outfitter_code(TEXT, UUID);
...
COMMENT ON FUNCTION validate_and_use_outfitter_code(TEXT, UUID, TEXT) IS
  'Validates outfitter code and links client to outfitter. p_email optional; when NULL uses auth.users email for p_user_id.';
